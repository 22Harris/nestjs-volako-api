...
export class DbBudgetRepository implements BudgetRepository {
  ...
  private async findWithLignes(id: number): Promise<Budget> {
    const r = await this.prisma.budget.findUnique({ where: { id }, include: { lignes: true } });
    return this.toEntity(r!);
  }

  findAll(): Promise<Budget[]> {
    return this.prisma.budget.findMany({ include: { lignes: true }, orderBy: [{ exercice: 'desc' }, { mois: 'desc' }] })
      .then(rs => rs.map(r => this.toEntity(r)));
  }

  async findByMois(exercice: number, mois: number): Promise<Budget | null> {
    const r = await this.prisma.budget.findFirst({ where: { exercice, mois }, include: { lignes: true } });
    return r ? this.toEntity(r) : null;
  }

  async create(exercice: number, mois: number): Promise<Budget> {
    const r = await this.prisma.budget.create({ data: { exercice, mois }, include: { lignes: true } });
    return this.toEntity(r);
  }
  ...
  async saveLigne(budgetId: number, ligne: any): Promise<Budget> {
    if (ligne.id) {
    ...
    return this.findWithLignes(budgetId);
  }

  async deleteLigne(budgetId: number, ligneId: number): Promise<Budget> {
